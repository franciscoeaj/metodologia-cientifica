...
Definindo a função de cálculo de sentimento por palavra

```{r}
# Faça você mesmo. Crie a variável polaridade_normalizada
calcular_polaridade_normalizada <- function (polaridade) {
     BEST_SENTIMENT = max(polaridade)
     WORST_SENTIMENT = min(polaridade)
     LIKERT_RANGE = 5
     
     # F(num) = likert - int([num + |min|] / likert) 
     normalize_function = LIKERT_RANGE - (as.integer((polaridade + abs(WORST_SENTIMENT)) / ((BEST_SENTIMENT + abs(WORST_SENTIMENT))/4)))
...
# Faça você mesmo. Crie a variável polaridade_normalizada
 reclamacoes_l = reclamacoes_l %>% 
     group_by(lexico) %>% 
     mutate(polaridade_normalizada = calcular_polaridade_normalizada(polaridade))
```
...
reclamacoes_tratado = reclamacoes_l %>% select(insatisfacao, avaliadores, range.avaliacoes, comprimento_reclamacao, palavras, polaridade, polaridade_normalizada, erro, lexico.dummy, orgao.dummy)

ggpairs(reclamacoes_tratado)
# lm1 = lm(<seu modelo>)
```
```{r}
model <- lm(formula = erro ~ lexico.dummy, data = reclamacoes_tratado)
summary(model)
plot(model)
```


O que podemos observar com este modelo: 

* Quando o léxico for 0, ou seja, op30, o erro aproximado é de 2.97 pontos. E ele diminui em -0.37 pontos quando utilizamos o léxico sent.
 
* Observando o desvio padrão, podemos concluir que quando o lexico utilizado for o op30, os erros podem variar em 0,54 pontos
 
* Observando o p-valor, concluímos que, com um valores bem próximos de 0, o intercept apresenta alta significância, enquanto o slope não apresenta. Então, a hipótese nula não é rejeitada e podemos concluir que há ouca ou nenhuma relação entre o erro e o léxico escolhido.
 
* Neste modelo, obtivemos um R² de 0.003892. Isso significa que apenas 0,04% dos erros obtidos são explicados pelo léxico escolhido.

```{r}
predict(model, reclamacoes_tratado, interval = "confidence")
```
